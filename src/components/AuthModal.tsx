...
export default function AuthModal() {
  const { auth } = Agnost.getBrowserClient();
  const { deleteByKey, has } = useSearchParams();

  const [selected, setSelected] = useState<number>();
  const redirectURL = env.NEXT_PUBLIC_APP_DOMAIN + "/auth-redirect";
  ...
  const loginMethods: LoginMethod[] = [
    {
      ...
      variant: "default",
      icon: GoogleIcon,
      handler: () => auth.signInWithProvider("google", redirectURL),
    },
    {
      name: "Github",
      variant: "light",
      icon: GithubIcon,
      handler: () => auth.signInWithProvider("github", redirectURL),
    },
  ];
